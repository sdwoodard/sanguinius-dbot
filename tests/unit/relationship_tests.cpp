...
TEST_CASE(
    "relationship policies are bounded and Tarot settlement hooks are active",
    "[relationship][policy]") {
  ...
  REQUIRE((
      sanguinius::relationship_policy(RelationshipSourceKind::tarot_resolved) ==
      sanguinius::RelationshipDelta{.familiarity = 1}));
  REQUIRE(
      (sanguinius::relationship_policy(RelationshipSourceKind::tarot_honored) ==
       sanguinius::RelationshipDelta{.esteem = 1, .reliability = 1}));
  REQUIRE((sanguinius::relationship_policy(
               RelationshipSourceKind::appearance_positive_feedback) ==
           sanguinius::RelationshipDelta{}));
  REQUIRE(
      (sanguinius::relationship_policy(RelationshipSourceKind::title_awarded) ==
       sanguinius::RelationshipDelta{.esteem = 1}));

  const auto bounded = sanguinius::apply_relationship_delta({.familiarity = 100,
                                                             .esteem = 0,
                                                             .mirth = 99,
                                                             .reliability = 1,
                                                             .wariness = 50},
                                                            {.familiarity = 5,
                                                             ...
                                                             .reliability = -5,
                                                             .wariness = 100});
  REQUIRE((bounded == sanguinius::RelationshipDimensions{.familiarity = 100,
                                                         .esteem = 0,
                                                         .mirth = 100,
                                                         .reliability = 0,
                                                         .wariness = 100}));
...
TEST_CASE("qualitative relationship bands cover every fixed threshold edge",
  ...
  const auto hint = sanguinius::relationship_style_hint({.familiarity = 15,
                                                         .esteem = 5,
                                                         .mirth = 1,
                                                         .reliability = 100,
                                                         .wariness = 100});
...
TEST_CASE("relationship recovery consumes at most fifty records per pass",
          "[relationship][orchestration][batch]") {
  sanguinius::test::FakeRelationshipRepository repository;
  repository.recovered_attempts = 30;
  repository.synchronized_sources = 40;
  sanguinius::test::FakeClock clock;
  sanguinius::test::FakePersistentIdGenerator ids;
  sanguinius::RelationshipService service{
      repository, clock, ids, sanguinius::ServerScopeConfiguration{10, 20, 30},
  ...
  REQUIRE(service.recover(50) == 50);
  REQUIRE(service.recover(50) == 20);
  REQUIRE(service.recover(50) == 0);
}
...
TEST_CASE("memory ranking requires relevance and is deterministic",
  ...
  std::vector<sanguinius::MemoryCandidate> candidates{
      {.memory_id = "b",
       .text = "The red dragon lives by the tower.",
       .tags = {"dragon"},
       .created_at_ms = now - 10,
       .revision = 1},
      {.memory_id = "a",
       .text = "The dragon hoard is beneath the keep.",
       .tags = {"dragon"},
       .created_at_ms = now - 10,
       .revision = 1},
      {.memory_id = "irrelevant",
       .text = "A quiet afternoon of gardening.",
       .tags = {"garden"},
       .created_at_ms = now,
       .revision = 1},
      {.memory_id = "reply",
       .text = "The ancient banner was restored.",
       .tags = {"banner"},
       .created_at_ms = now - 40LL * 24 * 60 * 60 * 1'000,
       .revision = 1},
  };
...
TEST_CASE("memory ranking applies every weight cap and relevance threshold",
  ...
  const auto ranked = sanguinius::rank_prompt_memories(
      ...
      "replytag replyone replytwo replythree replyfour replyfive replysix",
      now);
...
TEST_CASE("memory ranking honors every recency boundary",
  ...
  const auto score_at_age = [now](const std::int64_t age_ms) {
    auto ranked =
        sanguinius::rank_prompt_memories({{.memory_id = "memory",
                                           .text = "dragon",
                                           .tags = {},
...
TEST_CASE("memory ranking enforces candidate result and byte budgets",
  ...
  candidates.push_back({.memory_id = "outside-candidate-window",
                        ...
                        .revision = 1});
  REQUIRE(
      sanguinius::rank_prompt_memories(std::move(candidates), "dragon", {}, 2)
          .empty());
  ...
  std::vector<sanguinius::MemoryCandidate> bounded{
      ...
      {.memory_id = "b",
       ...
       .revision = 1},
      {.memory_id = "c",
       .text = text_of_size(700),
       .tags = {},
       .created_at_ms = 2,
       .revision = 1},
      {.memory_id = "d",
       .text = text_of_size(10),
       .tags = {},
       .created_at_ms = 2,
       .revision = 1},
  };
  const auto selected =
      sanguinius::rank_prompt_memories(std::move(bounded), "dragon", {}, 2);
  ...
  for (std::size_t index = 0; index < 5; ++index) {
    result_limited.push_back({.memory_id = std::to_string(index),
...
TEST_CASE("prompt compiler isolates hostile context from trusted instructions",
  ...
  const auto private_style =
      sanguinius::relationship_style_hint({.familiarity = 73,
                                           .esteem = 42,
                                           .mirth = 19,
                                           .reliability = 87,
                                           .wariness = 96});
  ...
  const auto reply_layer =
      request.conversation[0].content.find("EXPLICITLY REPLIED-TO MESSAGE");
