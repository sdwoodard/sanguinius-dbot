...
[[nodiscard]] std::int64_t scalar(
    sanguinius::persistence::SqliteRepositoryContext &context,
...
public:
  RelationshipFixture() {
    ...
    context = std::make_shared<sanguinius::persistence::SqliteRepositoryContext>(
    ...
    context->connection().execute(
        "UPDATE user_preference SET chronicle_opt_in=1 WHERE user_id IN ('31','32')");
    ...
    instances.record_start({.instance_id = uuid(1),
                            ...
                            .process_id = 1,
                            .started_at_ms = 100});
    chronicle = std::make_unique<
        sanguinius::persistence::SqliteChronicleRepository>(context);
    relationships = std::make_unique<
        sanguinius::persistence::SqliteRelationshipRepository>(context);
  }

  void confirm_memory(const std::size_t base, const std::string &text,
...
TEST_CASE("prompt attempts reserve relevant requester memories and finalize atomically",
  ...
  fixture.confirm_memory(110, "A peaceful garden blooms in spring.",
                         {"garden"});
  fixture.confirm_memory(120, "OTHER_USER_PRIVATE_SENTINEL dragon",
                         {"dragon"}, 32);
  fixture.confirm_memory(140, "EXPIRED_SENTINEL dragon", {"dragon"}, 31,
                         1'500);
  ...
  fixture.context->connection().execute(
      ...
      "memory_id='" +
      uuid(150) + "'");
  REQUIRE(fixture.chronicle->confirm_memory({
              .memory_id = uuid(130),
              .event_id = uuid(131),
  ...
  REQUIRE(scalar(*fixture.context,
                 "SELECT count(*) FROM memory_subject WHERE "
                 "memory_id='" + uuid(100) + "' AND subject_type='topic'") ==
          2);
  fixture.context->connection().execute(
  ...
  REQUIRE(prepared.memories[0].memory.text.find("crimson") != std::string::npos);
  ...
  REQUIRE(fixture.relationships->fail_prompt_attempt(
              {.attempt_id = uuid(205),
               .outcome = "cancelled",
  ...
  REQUIRE(scalar(*fixture.context,
                 "SELECT count(*) FROM relationship_event") == 1);
...
TEST_CASE("failed and privacy-invalidated attempts consume no social state",
  ...
  REQUIRE(scalar(*fixture.context,
                 "SELECT use_count FROM memory WHERE memory_id='" + uuid(300) +
                     "'") == 0);
  REQUIRE(scalar(*fixture.context,
                 "SELECT count(*) FROM relationship_event") == 0);
...
TEST_CASE("opted-out in-scope mentions remain source-idempotent without social state",
  ...
  const auto prepared =
      fixture.relationships->prepare_prompt_context(request);
  REQUIRE(prepared.status == sanguinius::PromptPreparationStatus::prepared);
...
TEST_CASE("relationship projections detect drift rebuild exactly and events stay immutable",
  ...
  fixture.context->connection().execute(
      "UPDATE relationship_state SET familiarity=99 WHERE subject_user_id='31'");
  ...
  fixture.context->connection().execute(
      ...
      "relationship_state BEGIN SELECT RAISE(ABORT,'injected rebuild failure'); "
      "END");
...
TEST_CASE("canon catch-up includes retracted source authors but excludes proposer-only users",
  ...
  REQUIRE(fixture.chronicle->retract_entry({
              .entity_id = uuid(500),
  ...
  REQUIRE(scalar(*fixture.context,
                 ...
                 "subject_user_id='32' AND reason_code='chronicle.canon'") == 1);
  REQUIRE(scalar(*fixture.context,
                 "SELECT count(*) FROM relationship_event WHERE "
...
TEST_CASE("restart recovery abandons only prior-instance prompt reservations",
  ...
  fixture.context->connection().execute(
      "UPDATE user_preference SET memory_callback_opt_in=0,updated_at_ms=150 "
      "WHERE user_id='31'");
  REQUIRE(fixture.relationships->prepare_prompt_context(
              fixture.prompt(600, "A pending request"))
  ...
  REQUIRE_THROWS(fixture.context->connection().execute(
      "DELETE FROM ai_prompt_attempt"));
  REQUIRE(scalar(*fixture.context,
                 "SELECT count(*) FROM relationship_event") == 0);
}

TEST_CASE("memory callback preferences are consent-gated audited and replay-safe",
          "[relationship][sqlite][privacy][idempotency]") {
  RelationshipFixture fixture;
...
TEST_CASE("competing prompt reservations and projection writes serialize safely",
