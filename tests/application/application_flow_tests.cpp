...
TEST_CASE("retired help and repository prefixes are silent",
          "[application][commands][retired-prefix]") {
  ...
  fixture.discord->emit(sanguinius::test::incoming("!HELP", 104));
  fixture.discord->emit(
      sanguinius::test::incoming("!repo trailing arguments", 105));
  ...
  const auto has_appearance_diagnostic = [&] {
    return std::ranges::any_of(
        fixture.diagnostics->events(), [](const auto &event) {
          return event.category == "message_observer.appearances";
        });
  };
...
TEST_CASE(
    "relationship projection drift does not block bounded startup recovery",
    "[application][relationships][orchestration][startup]") {
  auto options = social_ai_options();
  sanguinius::test::ApplicationFixture fixture{options};
  fixture.relationships->projection.valid = false;
