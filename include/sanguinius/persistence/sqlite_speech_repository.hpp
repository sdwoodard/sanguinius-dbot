...
public:
  ...
  claim_next(std::string_view voice_session_id, std::int64_t now_ms,
             std::string transition_id, std::string idempotency_key) override;
  ...
  void ensure_purge_schedule(std::int64_t now_ms, std::string job_id) override;
  ...
  [[nodiscard]] SpeechMutationStatus
  release_usage(std::string_view attempt_id) override;
  ...
  [[nodiscard]] std::string selected_voice(std::string_view guild_id) override;
  [[nodiscard]] SpeechMutationStatus
  select_voice(std::string_view guild_id, std::string_view voice,
               std::string_view actor_user_id, std::int64_t now_ms) override;
  [[nodiscard]] SpeechRepositoryHealth
  health(std::int64_t now_ms, std::int64_t calendar_month_start_ms) override;
