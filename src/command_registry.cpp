        ...
        .subcommands =
            {
                ...
                {"wager",
                 ...
                 {{CommandOptionKind::user, "target", "The other participant.", true},
                  {CommandOptionKind::string, "visibility", "Public or sealed offer.", false, 6, 6,
                   {{"Public", "public"}, {"Sealed", "sealed"}}},
                  {CommandOptionKind::string, "resolution", "Mutual or designated judgment.", false, 6, 10,
                   {{"Mutual", "mutual"}, {"Designated judge", "designated"}}},
                  {CommandOptionKind::user, "judge", "Required for designated judgment.", false},
                  {CommandOptionKind::string, "outcome-in", "Outcome window after acceptance.", false, 2, 3,
                   {{"1 hour", "1h"}, {"6 hours", "6h"}, {"24 hours", "24h"},
                    {"72 hours", "72h"}, {"7 days", "7d"}}}}},
                {"wagers", "Show your recent wagers or one exact wager.",
                 {{CommandOptionKind::string, "reference", "Optional full wager UUID.", false, 36, 36}}},
                {"wager-action", "Accept, decline, cancel, agree, dispute, or void.",
                 {{CommandOptionKind::string, "reference", "Full wager UUID.", true, 36, 36},
                  {CommandOptionKind::string, "action", "Authorized wager action.", true, 4, 7,
                   {{"Accept", "accept"}, {"Decline", "decline"}, {"Cancel", "cancel"},
                    {"Agree", "agree"}, {"Dispute", "dispute"}, {"Void", "void"}}}}},
                {"outcome", "Submit your proposed wager winner.",
                 {{CommandOptionKind::string, "reference", "Full wager UUID.", true, 36, 36},
                  {CommandOptionKind::string, "winner", "Creator or target.", true, 6, 7,
                   {{"Creator", "creator"}, {"Target", "target"}}}}},
                {"evidence", "Append private participant evidence.",
                 {{CommandOptionKind::string, "reference", "Full wager UUID.", true, 36, 36},
                  {CommandOptionKind::string, "evidence", "Private evidence text.", true, 1, 1000}}},
                {"judgment", "Submit an authorized reasoned judgment.",
                 {{CommandOptionKind::string, "reference", "Full wager UUID.", true, 36, 36},
                  {CommandOptionKind::string, "result", "Creator, target, or void.", true, 4, 7,
                   {{"Creator", "creator"}, {"Target", "target"}, {"Void", "void"}}},
                  {CommandOptionKind::string, "reason", "Bounded judgment reason.", true, 1, 200}}},
                {"disputes", "Show wager disputes you are authorized to view.",
                 {{CommandOptionKind::string, "reference", "Optional full wager UUID.", false, 36, 36}}},
            },
    });
  }
      ...
      owner_controls.subcommand_groups.push_back(
          CommandSubcommandGroupDefinition{
              ...
              .subcommands = {
                  ...
                  CommandSubcommandDefinition{
                      .name = "wager-role",
                      .description = "Select a simulated role for a [TEST] wager.",
                      .options = {
                          {CommandOptionKind::string, "reference", "Full test wager UUID.", true, 36, 36},
                          {CommandOptionKind::string, "role", "Role to simulate.", true, 5, 7,
                           {{"Creator", "creator"}, {"Target", "target"},
                            {"Judge", "judge"}, {"Owner", "owner"}}}}},
                  CommandSubcommandDefinition{
                      .name = "wager-deadline",
                      .description = "Force one [TEST] wager deadline phase.",
                      .options = {
                          {CommandOptionKind::string, "reference", "Full test wager UUID.", true, 36, 36},
                          {CommandOptionKind::string, "phase", "Deadline phase.", true, 5, 8,
                           {{"Draft expiry", "draft"}, {"Offer expiry", "offer"},
                            {"Reminder", "reminder"}, {"Outcome due", "outcome"},
                  ...
                  CommandSubcommandDefinition{
                      ...
                      .options = {
                          {CommandOptionKind::string, "reference", "Full test wager UUID.", true, 36, 36},
                          {CommandOptionKind::string, "reason", "Audited cleanup reason.", true, 1, 200}}},
              }});
    }
  }
