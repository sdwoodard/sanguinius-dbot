...
overlap_count(const std::vector<std::string> &left,
...
[[nodiscard]] std::size_t
tag_overlap_count(const std::vector<std::string> &tags,
  ...
  for (const auto &tag : tags) {
    ...
    if (!tag_tokens.empty() &&
        std::ranges::all_of(tag_tokens,
                            [&query_tokens](const auto &token) {
...
[[nodiscard]] std::string_view dimension_phrase(const std::string_view kind,
  ...
  if (kind == "familiarity") {
    switch (band) {
    case QualitativeBand::dormant:
      return "a bond not yet written";
    case QualitativeBand::emerging:
      return "newly acquainted";
    case QualitativeBand::established:
      return "familiar company";
    case QualitativeBand::strong:
      return "a trusted companion";
    case QualitativeBand::storied:
      return "a storied ally";
    case QualitativeBand::legendary:
      return "an inner-circle bond";
    }
  }
  if (kind == "esteem") {
    switch (band) {
    case QualitativeBand::dormant:
      return "deeds not yet weighed";
    case QualitativeBand::emerging:
      return "deeds beginning to be noticed";
    case QualitativeBand::established:
      return "respected deeds";
    case QualitativeBand::strong:
      return "honored deeds";
    case QualitativeBand::storied:
      return "renowned deeds";
    case QualitativeBand::legendary:
      return "legendary esteem";
    }
  }
  if (kind == "mirth") {
    switch (band) {
    ...
    case QualitativeBand::emerging:
      return "the first shared jokes";
    case QualitativeBand::established:
      return "easy banter";
    case QualitativeBand::strong:
      return "well-practiced mischief";
    case QualitativeBand::storied:
      return "notorious shared chaos";
    case QualitativeBand::legendary:
      return "mythic levels of mischief";
    }
  }
  if (kind == "reliability") {
    switch (band) {
    case QualitativeBand::dormant:
      return "steadfastness not yet tested";
    case QualitativeBand::emerging:
      return "early signs of follow-through";
    case QualitativeBand::established:
      return "proven follow-through";
    case QualitativeBand::strong:
      return "steadfast reliability";
    case QualitativeBand::storied:
      return "unyielding reliability";
    case QualitativeBand::legendary:
      return "reliability beyond question";
    }
  }
  switch (band) {
  case QualitativeBand::dormant:
    return "no cause for narrative suspicion";
  case QualitativeBand::emerging:
    return "a faint, playful suspicion";
  case QualitativeBand::established:
    return "watchful amusement";
  case QualitativeBand::strong:
    return "a known agent of chaos";
  case QualitativeBand::storied:
    return "legendary cause for watchfulness";
  case QualitativeBand::legendary:
    return "destiny itself is watching";
  }
  return "unwritten";
}

[[nodiscard]] std::string safe_reason(const std::string_view reason) {
  if (reason == "chronicle.canon")
    return "A shared moment entered the Chronicle.";
  if (reason == "ai.direct")
    return "Sanguinius answered a direct call.";
  if (reason == "ai.direct_cooldown")
    return "A recent conversation continued the bond.";
  if (reason == "tarot.resolved")
    return "A wager reached its conclusion.";
  if (reason == "tarot.honored")
    return "A wager was honorably fulfilled.";
  if (reason == "appearance.positive_feedback")
    return "A well-timed appearance earned a smile.";
  if (reason == "title.awarded")
    return "A title marked a notable deed.";
  if (reason == "session.completed")
    return "A shared Chronicle session was completed.";
...
RelationshipDelta
relationship_policy(const RelationshipSourceKind kind,
...
apply_relationship_delta(const RelationshipDimensions current,
  ...
  return {.familiarity =
              clamp_dimension(current.familiarity + requested.familiarity),
          .esteem = clamp_dimension(current.esteem + requested.esteem),
          .mirth = clamp_dimension(current.mirth + requested.mirth),
          .reliability =
              clamp_dimension(current.reliability + requested.reliability),
...
QualitativeBand qualitative_band(const int value) noexcept {
  if (value <= 0)
    return QualitativeBand::dormant;
  if (value <= 4)
    return QualitativeBand::emerging;
  if (value <= 14)
    return QualitativeBand::established;
  if (value <= 29)
    return QualitativeBand::strong;
  if (value <= 59)
    return QualitativeBand::storied;
...
std::string relationship_style_hint(const RelationshipDimensions &dimensions) {
  ...
  if (dimensions.esteem > 0) {
    hints.push_back(
        dimension_phrase("esteem", qualitative_band(dimensions.esteem)));
  }
  if (dimensions.mirth > 0) {
    hints.push_back(
        dimension_phrase("mirth", qualitative_band(dimensions.mirth)));
  }
  if (hints.empty())
    return {};
  ...
  for (std::size_t index = 0; index < hints.size(); ++index) {
    if (index != 0)
      result += index + 1 == hints.size() ? " and " : ", ";
    result += hints[index];
  }
  result +=
      ". Treat this only as private style guidance; never state or explain it.";
  return result;
}
...
rank_prompt_memories(std::vector<MemoryCandidate> candidates,
  ...
  for (auto &candidate : candidates) {
    ...
    const auto replied_text_matches =
        overlap_count(replied_tokens, memory_set, 5);
    ...
    const auto signals =
        current_text + replied_text_matches + current_tags + replied_tags;
    if (signals == 0)
      continue;
    int recency = 0;
    const auto age =
        std::max<std::int64_t>(0, now_ms - candidate.created_at_ms);
    if (age <= 30LL * 24 * 60 * 60 * 1'000)
      recency = 12;
    else if (age <= 90LL * 24 * 60 * 60 * 1'000)
      recency = 8;
    else if (age <= 365LL * 24 * 60 * 60 * 1'000)
      recency = 4;
    const int score =
        static_cast<int>(current_tags * 40 + current_text * 12 +
  ...
  std::ranges::sort(
      ranked, [](const RankedMemory &left, const RankedMemory &right) {
  ...
  for (auto &item : ranked) {
    if (selected.size() == maximum_prompt_memories)
      break;
    if (bytes + item.memory.text.size() > maximum_prompt_memory_bytes)
      continue;
...
RelationshipService::RelationshipService(RelationshipRepository &repository,
                                         const Clock &clock,
...
RelationshipService::prepare_prompt(const IncomingMessage &message,
                                    std::string current_request,
                                    std::string replied_text) {
  if (!in_feature_scope(message))
    return {};
...
RelationshipService::fail_prompt(const std::string_view attempt_id,
  ...
  return repository_.fail_prompt_attempt({.attempt_id = std::string{attempt_id},
...
RelationshipService::profile(const IncomingInteraction &interaction) {
  const auto target =
      user_option(interaction, "user").value_or(interaction.user_id);
  const bool public_view = target != interaction.user_id;
  const auto data = repository_.profile(interaction.user_id, target,
                                        public_view, unix_milliseconds(clock_));
  if (!data.found || data.is_bot || !data.chronicle_opt_in) {
    return text_message(public_view
                            ? "No public Chronicle profile is available."
  ...
  if (public_view) {
    ...
    if (data.latest_session_summary)
      output << "\nLatest chapter: " << *data.latest_session_summary;
    for (const auto &title : data.visible_canon_titles)
      output << "\n- " << title;
  ...
  output << "**Your Chronicle profile**\n"
         << "Bond: "
         << dimension_phrase("familiarity",
                             qualitative_band(data.dimensions.familiarity))
         << "\nRegard: "
         << dimension_phrase("esteem", qualitative_band(data.dimensions.esteem))
         << "\nBanter: "
         << dimension_phrase("mirth", qualitative_band(data.dimensions.mirth))
         << "\nSteadfastness: "
         << dimension_phrase("reliability",
                             qualitative_band(data.dimensions.reliability))
         << "\nNarrative caution: "
         << dimension_phrase("wariness",
                             qualitative_band(data.dimensions.wariness))
         << "\nMemory callbacks: "
         << (data.memory_callbacks ? "enabled" : "disabled");
  if (data.featured_title)
    output << "\nFeatured title: **" << *data.featured_title << "**";
  if (data.latest_session_summary)
    output << "\nLatest chapter: " << *data.latest_session_summary;
  ...
  if (!data.recent_reasons.empty()) {
    output << "\nRecent continuity:";
    for (const auto &reason : data.recent_reasons)
      output << "\n- " << safe_reason(reason);
  }
  if (!data.visible_canon_titles.empty()) {
...
InteractionMessage RelationshipService::set_memory_callbacks(
  ...
  const auto result = repository_.set_memory_callbacks({
      ...
      .idempotency_key =
          "chronicle:callbacks:" + interaction.interaction_id.str(),
      .now_ms = unix_milliseconds(clock_),
  });
  if (result == PreferenceChangeStatus::chronicle_opted_out) {
    return text_message(
        "Chronicle participation must be enabled before memory callbacks.");
  }
  if (result == PreferenceChangeStatus::unchanged) {
    return text_message(*mode == "on"
                            ? "Memory callbacks are already enabled."
...
std::size_t RelationshipService::recover(const std::size_t limit) {
  if (limit == 0 || limit > 50)
    throw std::invalid_argument{"Relationship recovery batch is invalid."};
  const auto now_ms = unix_milliseconds(clock_);
  const auto attempts = repository_.recover_prompt_attempts(
      application_instance_id_, now_ms, limit);
  if (attempts >= limit)
    return attempts;
