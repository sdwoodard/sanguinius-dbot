...
| `/sang-admin appearance trigger fixture:owner_live_safe` | Owner-only, admin/test-mode-gated, visibly tagged one-person live delivery through the normal transaction and outbox. Configured mode must be `live`. Because its payload is fixed and synthetic, the acceptance trigger ignores prior bot-speech recency; mode, kill, quiet, consent, scope, readiness, budget, cooldown, and delivery gates remain enforced. |
